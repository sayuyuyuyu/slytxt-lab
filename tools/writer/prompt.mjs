...
/** note 用。誤字だけを直し、内容には触れない。 */
export async function notePrompt(draft) {
  return `短いメモを、誤字だけ直してそのまま公開できる形にする。
...
- 見出しを増やさない。前置き、結び、総括を足さない。
- 行を増やさない。書いてあることだけを、正しい字で残す。

## 出力

本文の Markdown だけを出力する。frontmatter は書かない。前置き、後始末、コードフェンスを書かない。
